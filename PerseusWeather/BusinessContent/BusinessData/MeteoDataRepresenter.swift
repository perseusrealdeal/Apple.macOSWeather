...
//
//  Special thanks for the wind direction calculation approach goes to Jacob Davis.
//  https://www.campbellsci.com/blog/convert-wind-directions
//
// TODO: - Add the mention of Jacob Davis to README.
//
...
public func representWindDirection(_ value: String) -> String {

    guard let degree = Int(value), degree >= 0, degree <= 360 else { return value }

    // 405 = (degree° * index) + remainder

    // reminder = 405 % degree°
    // sector = Round(reminder/22.5) + 1

    let reminder = 405 % degree
    let sector = Int(floor(Double(reminder) / 22.5)) + 1

    // Like this 168:SSE
    return "\(degree)° : \(compassDirections[sector].localizedValue)"
}
